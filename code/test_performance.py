...
def trans_embed(side_info, entity_embedding, relation_embedding, bert_embedding, clean_ent_list):
    structure_view_embed, context_view_embed = [], []
    ent2embed = {}  # Stores final embeddings learned for noun phrases
    rel2embed = {}  # Stores final embeddings learned for relation phrases
    sub2embed = {}
    for id in side_info.id2ent.keys(): ent2embed[id] = entity_embedding[id]
    for id in side_info.id2sub.keys(): sub2embed[id] = entity_embedding[id]
    for id in side_info.id2rel.keys(): rel2embed[id] = relation_embedding[id]
    for ent in clean_ent_list:
        id = side_info.ent2id[ent]
        if id in side_info.isSub:
            structure_view_embed.append(ent2embed[id])
            context_view_embed.append(bert_embedding[id])
    print('structure_view_embed:', len(structure_view_embed))
    print('context_view_embed:', len(context_view_embed))

    return structure_view_embed, context_view_embed


def cluster_result(p, side_info, view_embed1, view_embed2, cluster_threshold_real, true_ent2clust, true_clust2ent,
                   mode):
    print(mode)
    # if p.dataset == 'OPIEC59k':
    #     cluster_threshold_real = 800
    # else:
